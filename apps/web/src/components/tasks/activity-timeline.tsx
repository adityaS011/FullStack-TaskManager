...
export function ActivityTimeline({ logs }: ActivityTimelineProps) {
  ...
  return (
    <ol className="space-y-2.5">
      {logs.map((log) => (
        <li className="rounded-lg border border-border bg-background p-3" key={log.id}>
...
function ActivityDetails({ log }: { log: ActivityLog }) {
  ...
  if (log.action === "attachment.added" || log.action === "attachment.deleted") {
    return (
      <div className="mt-2 rounded-md bg-surface px-3 py-2 text-sm text-muted-foreground">
